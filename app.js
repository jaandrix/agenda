        ...
        :root {
            ...
            --badge-pending-bg: #2a2a2a;
            --badge-pending-text: #e5e5e5;
            --badge-progress-bg: #3a3a3a;
            --badge-progress-text: #ffffff;
            --badge-done-bg: #ffffff;
            --badge-done-text: #000000;
            --badge-default-bg: #262626;
            --badge-default-text: #e5e5e5;
            --shadow: 0 4px 20px rgba(0,0,0,0.45);
            --radius: 20px;
            --radius-full: 999px;
            --fantasy-accent: #8B5CF6;
            --fantasy-accent-soft: #1a152e;
            --vault-accent: #F59E0B;
        ...
        body.light {
            ...
            --badge-pending-bg: #ededea;
            --badge-pending-text: #3d3d3d;
            --badge-progress-bg: #e2e2df;
            --badge-progress-text: #1a1a1a;
            --badge-done-bg: #000000;
            --badge-done-text: #ffffff;
            --badge-default-bg: #ededea;
            --badge-default-text: #1a1a1a;
            --shadow: 0 4px 20px rgba(0,0,0,0.08);
            --fantasy-accent: #7C3AED;
            --fantasy-accent-soft: #ede9fe;
            --vault-accent: #D97706;
        ...
        body.beige {
            ...
            --bg-sidebar: #ece1cb;
            --bg-card: #f7f0e2;
            --bg-card-hover: #efe4cd;
            --bg-input: #ece0c8;
            --bg-modal: #f7f0e2;
            --bg-fab: #6b5c3f;
            --border: #ddccaa;
            --border-strong: #cdb98c;
            --text-primary: #3d3324;
            --text-secondary: #8a7a5c;
            --text-muted: #b5a68a;
            --accent: #6b5c3f;
            --accent-contrast: #f7f0e2;
            ...
            --badge-default-bg: #ddccaa;
            --badge-default-text: #3d3324;
            --shadow: 0 4px 20px rgba(60,50,30,0.15);
            --fantasy-accent: #7C3AED;
            --fantasy-accent-soft: #ede9fe;
            --vault-accent: #D97706;
            --vault-accent-soft: #fef3c7;
        ...
        * {
        ...
        /* Fantasy nunca se pinta durante el arranque: solo se revela cuando
           el modo desarrollador se activa explícitamente. */
        .fantasy-hidden { display:none !important; }
        ...
        .sidebar-nav button.fantasy-btn {
            color: var(--fantasy-accent);
        }
        .sidebar-nav button.fantasy-btn:hover {
            background: var(--fantasy-accent-soft);
            color: var(--fantasy-accent);
        }
        .sidebar-nav button.fantasy-btn.active {
            background: var(--fantasy-accent-soft);
            color: var(--fantasy-accent);
        }
        ...
        #mobile-menu-panel .menu-nav button.fantasy-btn {
            color: var(--fantasy-accent);
        }
        #mobile-menu-panel .menu-nav button.fantasy-btn:hover {
            background: var(--fantasy-accent-soft);
            color: var(--fantasy-accent);
        }
        #mobile-menu-panel .menu-nav button.fantasy-btn.active {
            background: var(--fantasy-accent-soft);
            color: var(--fantasy-accent);
        }
        ...
        .notif-item-title { font-size: 12.5px; font-weight: 600; color: var(--text-primary); line-height: 1.35; }
        ...
        .weekly-tasks-title { font-size: 14px; font-weight: 800; letter-spacing: -.2px; }
        .weekly-tasks-week { margin-top: 2px; font-size: 10px; color: #77736a; }
        .weekly-tasks-add {
        ...
        .weekly-tasks-add:hover { background: #ffffff; border-color: rgba(0,0,0,.24); }
        .weekly-tasks-list {
        ...
        .weekly-task-row:hover { background: rgba(0,0,0,.035); }
        .weekly-task-check {
            appearance: none;
            ...
            flex: 0 0 17px;
            margin: 0;
            border: 1.5px solid #8b877d;
            border-radius: 5px;
            background: transparent;
            cursor: pointer;
            position: relative;
        }
        .weekly-task-check:hover { border-color: #333; }
        .weekly-task-text {
            min-width: 0;
            font-size: 12px;
            line-height: 1.35;
        ...
        .cal-strip-letter { font-size: 10px; font-weight: 700; text-transform: uppercase; opacity: 0.7; }
        .cal-strip-num { font-size: 14px; font-weight: 700; }
        ...
        .cal-week-col-num { font-size: 15px; font-weight: 800; color: var(--text-primary); }
        ...
        /* ============================================================
               EFECTO HOVER GLOBAL: icono de edición
               Grupo A: tarjetas normales -> solo aparece el lápiz,
               con un anillo azul Bitácora grueso y sombra para
               distinguirlo bien.
               No se aplica en Centro resumen (usa showDayEntries, que
               queda fuera a propósito).
            ============================================================ */
        div[onclick*="openEntryDetail"],
        div[onclick*="openReadNote"],
        div[onclick*="openFundDetail"],
        div[onclick*="openPromptDetail"],
        div[onclick*="openEditEntry"] {
            position: relative;
        }
        div[onclick*="openEntryDetail"]::after,
        div[onclick*="openReadNote"]::after,
        div[onclick*="openFundDetail"]::after,
        div[onclick*="openPromptDetail"]::after,
        div[onclick*="openEditEntry"]::after {
            content: '✎';
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        div[onclick*="openEntryDetail"]:hover::after,
        div[onclick*="openReadNote"]:hover::after,
        div[onclick*="openFundDetail"]:hover::after,
        div[onclick*="openPromptDetail"]:hover::after,
        div[onclick*="openEditEntry"]:hover::after {
            opacity: 1;
            transform: translate(-50%,-50%) scale(1);
        }

        /* Grupo B: tarjetas negras (Coleccionables, Libros, Series,
           Películas, Videojuegos) -> difuminado + lápiz con círculo
           blanco/icono negro, siempre visible sobre el fondo oscuro
           de la tarjeta. */
        .media-card.media-card-type-book,
        .media-card.media-card-type-movie,
        .media-card.media-card-type-series,
        .media-card.media-card-type-game,
        .collectible-card {
            position: relative;
            overflow: hidden;
        }
        ...
        .media-card.media-card-type-book:hover > *,
        .media-card.media-card-type-movie:hover > *,
        .media-card.media-card-type-series:hover > *,
        .media-card.media-card-type-game:hover > *,
        .collectible-card:hover > * {
            filter: blur(3px);
            opacity: 0.4;
        }
        .media-card.media-card-type-book::after,
        .media-card.media-card-type-movie::after,
        .media-card.media-card-type-series::after,
        .media-card.media-card-type-game::after,
        ...
        .media-card.media-card-type-book:hover::after,
        .media-card.media-card-type-movie:hover::after,
        .media-card.media-card-type-series:hover::after,
        .media-card.media-card-type-game:hover::after,
        .collectible-card:hover::after {
            opacity: 1;
            transform: translate(-50%,-50%) scale(1);
        }
        ...
        body.light .entry-actions button.delete:hover {
            background: #fee2e2;
        }
        body.beige .entry-actions button.delete:hover {
            background: #f3e0d8;
        }
        ...
        .command-palette-prompt { font-size: 14px; font-weight: 700; color: var(--text-secondary); margin-bottom: 12px; }
        .command-palette-prompt .cp-accent { color: #3b82f6; }
        .command-palette-input {
            width: 100%; border: none; background: transparent; outline: none;
            font-size: 21px; font-weight: 700; color: #3b82f6;
            font-family: inherit; padding: 4px 0 14px; border-bottom: 1px solid var(--border);
        }
        .command-palette-input::placeholder { color: var(--text-secondary); font-weight: 600; }
        ...
        .command-palette-item {
            ...
            border-radius: 14px; cursor: pointer; font-size: 14px; font-weight: 600; color: var(--text-primary);
            transition: background 0.12s ease;
        }
        ...
        .centered-message-text { font-size: 16px; font-weight: 700; color: var(--text-primary); margin-bottom: 18px; }
        ...
        .daily-alert-sheet {
            ...
            animation: dailyAlertIn 0.45s cubic-bezier(0.16,1,0.3,1) both;
        }
        .daily-alert-inner { background: #0a0a0c; border-radius: 24px; padding: 26px 22px; }
        .daily-alert-kicker { font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: #60a5fa; }
        .daily-alert-title { font-size: 22px; font-weight: 800; color: #fff; margin: 4px 0 18px; }
        .daily-alert-group { margin-bottom: 14px; }
        .daily-alert-group-label { font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: rgba(255,255,255,0.5); margin-bottom: 6px; }
        .daily-alert-item { padding: 9px 0; border-bottom: 1px solid rgba(255,255,255,0.08); color: #fff; font-size: 13px; cursor: default; }
        .daily-alert-item:last-child { border-bottom: none; }
        .daily-alert-item[onclick] { cursor: pointer; }
        .daily-alert-item[onclick]:hover { color: #93c5fd; }
        .daily-alert-close { width: 100%; margin-top: 10px; padding: 12px; border: 0; border-radius: 16px; background: #fff; color: #000; font-weight: 800; cursor: pointer; }
        .modal-overlay {
            position: fixed;
            inset: 0;
            background: rgba(0, 0, 0, 0.6);
        ...
        body.light .btn-modal-danger {
            background: #fee2e2;
        }
        body.beige .btn-modal-danger {
            background: #f3e0d8;
        }
        ...
        .friend-add-input::placeholder { letter-spacing: normal; font-family: 'Poppins', sans-serif; }
        ...
        .friend-list-name { font-size: 13px; font-weight: 600; color: var(--text-primary); }
        .friend-remove-btn {
            width: 24px;
            height: 24px;
            flex-shrink: 0;
        ...
        .about-feature-title { font-size: 13px; font-weight: 800; color: var(--text-primary); margin-bottom: 3px; }
        ...
        .about-footer-title { font-size: 13px; font-weight: 800; color: var(--text-primary); margin-bottom: 4px; }
        ...
        .year-cal-month-label { width: 34px; flex-shrink: 0; font-size: 10px; font-weight: 700; color: var(--text-secondary); text-transform: capitalize; }
        ...
        /* ============================================================
               FANTASY
            ============================================================ */
        /* Variables temáticas de Fantasy: por defecto heredan el tema general
           de la app; con la clase .fantasy-theme-green se sustituyen por una
           gama oscura con tonalidades verdes/teal, sin tocar el resto de la app. */
        .fantasy-root {
            --fx-bg: var(--bg-card);
            --fx-bg-hover: var(--bg-card-hover);
            --fx-border: var(--border);
            --fx-accent: var(--fantasy-accent);
            --fx-accent-soft: var(--fantasy-accent-soft);
            --fx-text: var(--text-primary);
            --fx-text-secondary: var(--text-secondary);
            --fx-page-bg: transparent;
            --fx-grid: rgba(255,255,255,0.06);
        }
        .fantasy-root.fantasy-theme-green {
            --fx-bg: #0e1f17;
            --fx-bg-hover: #14291f;
            --fx-border: #1f3d2e;
            --fx-accent: #22d3aa;
            --fx-accent-soft: #0e2a20;
            --fx-text: #eafff6;
            --fx-text-secondary: #7fa695;
            --fx-page-bg: #070f0b;
            --fx-grid: rgba(34,211,170,0.10);
        }
        /* El tinte verde de fondo se aplica a toda la ventana mediante
           body.fantasy-green-page (ver toggleFantasyTheme/render), no solo
           a los bloques de Fantasy. */
        body.fantasy-green-page {
            background: #070f0b;
        }

        .fantasy-positive { color: #16a34a; }
        .fantasy-negative { color: #dc2626; }
        ...
        .fantasy-more-menu.open {
            display: flex;
            flex-direction: column;
            gap: 4px;
        }
        .fantasy-more-menu .btn-secondary {
            margin: 0;
            width: 100%;
            text-align: left;
            padding: 7px 10px;
        ...
        .fantasy-more-menu .fantasy-theme-toggle {
            display: flex;
            align-items: center;
            justify-content: space-between;
            gap: 8px;
        ...
        .fantasy-switch input {
            opacity: 0;
            width: 0;
            height: 0;
        }
        .fantasy-switch-track {
            position: absolute;
            inset: 0;
        ...
        .fantasy-switch input:checked + .fantasy-switch-track {
            background: var(--fx-accent);
        }
        .fantasy-switch input:checked + .fantasy-switch-track::before {
        ...
        .fantasy-user-card:hover {
            background: var(--fx-bg-hover);
            border-color: var(--fx-accent);
            box-shadow: 0 6px 16px rgba(0,0,0,0.18);
            transform: translateY(-1px);
        }
        .fantasy-user-card .user-avatar {
            width: 30px;
            height: 30px;
            border-radius: 50%;
            display: flex;
        ...
        .fantasy-user-card .user-meta {
            font-size: 10px;
            color: var(--fx-text-secondary);
            margin-top: 2px;
        }
        .fantasy-user-card .user-actions {
            display: flex;
            flex-wrap: wrap;
            justify-content: center;
            gap: 4px;
        ...
        .fantasy-user-card .user-actions .btn-secondary {
            margin: 0;
            padding: 3px 7px;
            font-size: 10px;
            width: auto;
        ...
        .fantasy-user-card .user-balance {
            display: flex;
            flex-direction: column;
            align-items: center;
        }
        .fantasy-user-card .user-balance .cash {
            font-weight: 800;
            font-size: 19px;
            line-height: 1.2;
            color: #16a34a;
        }
        .fantasy-user-card .user-balance .cash.fantasy-negative {
            color: #dc2626;
        }
        .fantasy-user-card .user-balance .cash-label {
            font-size: 10px;
            font-weight: 700;
            text-transform: uppercase;
            letter-spacing: 0.5px;
        ...
        .fantasy-user-card .user-balance .total {
            font-size: 11px;
            color: var(--fx-text-secondary);
            margin-top: 4px;
        }
        ...
        .fantasy-tx-item:last-child {
            border-bottom: none;
        }
        .fantasy-tx-item .tx-type {
            font-weight: 700;
            text-transform: uppercase;
            font-size: 11px;
        }
        .fantasy-tx-item .tx-type.buy {
            color: var(--fx-accent);
        }
        .fantasy-tx-item .tx-type.sell {
            color: #16a34a;
        }
        .fantasy-tx-item .tx-amount {
            font-weight: 600;
        }
        .fantasy-tx-item .tx-amount.positive {
            color: #16a34a;
        }
        .fantasy-tx-item .tx-amount.negative {
            color: #dc2626;
        }
        ...
        .fantasy-chart-container:hover {
            box-shadow: 0 6px 16px rgba(0,0,0,0.14);
        }
        .fantasy-chart-container .fantasy-canvas-wrap {
            position: relative;
            width: 100%;
            height: 220px;
        }
        .fantasy-chart-container canvas {
            max-width: 100%;
        }
        .fantasy-chart-header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            flex-wrap: wrap;
        ...
        .fantasy-chart-header .fantasy-section-title {
            margin-bottom: 0;
        }
        ...
        .fantasy-user-chip {
            ...
            padding: 4px 9px;
            border-radius: 999px;
            border: 1px solid var(--chip-color, var(--fx-accent));
            background: rgba(139,92,246,0.12);
            background: color-mix(in srgb, var(--chip-color, var(--fx-accent)) 16%, transparent);
            color: var(--fx-text);
            font-size: 11px;
            font-weight: 600;
            cursor: pointer;
        ...
        .fantasy-user-chip::before {
            content: '';
            width: 7px;
            height: 7px;
            border-radius: 50%;
            background: var(--chip-color, var(--fx-accent));
            flex-shrink: 0;
        }
        .fantasy-user-chip.off {
            opacity: 0.35;
            background: transparent;
        }
        .fantasy-user-chip.all {
            border-style: dashed;
        }
        ...
        .fantasy-profit-table {
            width: 100%;
            border-collapse: collapse;
            font-size: 12px;
        }
        .fantasy-profit-table th {
            text-align: left;
            font-size: 10px;
            font-weight: 700;
            text-transform: uppercase;
        ...
        .fantasy-profit-table td {
            padding: 7px 8px;
            border-bottom: 1px solid var(--fx-border);
            white-space: nowrap;
        }
        .fantasy-profit-table tr:last-child td {
            border-bottom: none;
        }
        .fantasy-profit-table .profit-user {
            font-weight: 700;
            color: var(--fx-text);
            cursor: pointer;
        }
        .fantasy-profit-table .profit-user:hover {
            color: var(--fx-accent);
        }
        ...
        #header-clock { font-family:'JetBrains Mono',monospace; font-size:12px; font-weight:600; color:var(--text-secondary); min-width:42px; text-align:right; }
...
.entry-detail-external {
    ...
    padding:0; font-family:'Poppins',sans-serif; font-size:12px; font-weight:700;
...
@media(max-width:600px){ .entry-detail-external{ position:static; margin-top:12px; } .entry-detail-title-with-external{ padding-right:0; } }
        ...
        .entry-detail-label { font-size:10px; text-transform:uppercase; letter-spacing:.6px; color:var(--text-secondary); margin-bottom:5px; font-weight:700; }
        ...
        .doc-item {
            ...
            border-color:var(--bone-border);
            color:var(--bone-text);
        }
        .doc-item .doc-name { color:var(--bone-text); }
        .doc-item .doc-meta { color:var(--bone-muted); }
        .doc-item .doc-actions button { color:var(--bone-text); border-color:var(--bone-border); }
        ...
        .trip-manager-title { font-size: 22px; font-weight: 800; margin-top: 8px; letter-spacing: -.3px; }
        ...
        .doc-action-download:hover { background: #e5e5e5; }
        ...
        .doc-action-delete-btn:hover { background: #fee2e2; }
        .doc-upload-box {
...
/* ============================================================
   ...
   ============================================================ */
.finance-dashboard { position: relative; max-width: 980px; padding-bottom: 30px; }
.finance-networth-value-row { display: flex; align-items: center; gap: 10px; }
.finance-blur-toggle {
    width: 38px; height: 38px; border-radius: 50%; flex-shrink: 0;
    border: none; background: #3b82f6; color: #fff;
    display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all .15s ease;
}
.finance-blur-toggle:hover { background: #2f6fd1; }
...
.finance-dashboard .finance-goal-bar-text,
...
.finance-dashboard.blurred .finance-goal-bar-text,
...
.finance-dashboard-head h2 { margin:2px 0 8px; font-size:38px; font-weight:800; letter-spacing:-1px; }
.finance-dashboard-head p { margin:0; color:var(--text-secondary); font-size:13px; max-width:520px; line-height:1.6; }
.finance-kicker { font-size:10px; text-transform:uppercase; letter-spacing:.8px; font-weight:800; color:var(--text-secondary); }
...
.finance-oneoff-btn { width:auto; white-space:nowrap; padding:10px 16px; border:0; border-radius:999px; background:var(--bg-card); color:var(--text-primary); font-size:11px; font-weight:750; cursor:pointer; transition:all .15s ease; box-shadow:inset 0 0 0 1px var(--border-strong); }
...
.finance-subnav { display:flex; gap:4px; flex-wrap:wrap; margin-top:14px; background:var(--bg-card); padding:5px; border-radius:999px; border:1px solid var(--border); width:fit-content; }
.finance-subnav button { border:0; background:none; color:var(--text-secondary); border-radius:999px; padding:7px 14px; font-size:10px; font-weight:700; cursor:pointer; transition:all .15s ease; }
...
.finance-networth-value { font-size:48px; line-height:1; font-weight:900; letter-spacing:-1.5px; margin:10px 0; }
...
.finance-networth-side strong { font-size:30px; font-weight:800; }
...
.finance-metrics-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-bottom:18px; }
.finance-metric-card { position:relative; background:var(--bg-card); border:3px solid #000; border-radius:20px; padding:22px 14px 18px; min-width:0; display:flex; flex-direction:column; align-items:center; text-align:center; gap:4px; box-shadow:0 6px 18px rgba(0,0,0,.18); }
.finance-metric-card:hover { box-shadow:0 8px 22px rgba(0,0,0,.26); }
.finance-panel-head { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; }
.finance-metric-icon { width:38px; height:38px; border-radius:50%; background:var(--bg-card-hover); display:flex; align-items:center; justify-content:center; font-size:15px; margin-bottom:6px; }
.finance-metric-label { order:3; font-size:10px; text-transform:uppercase; font-weight:800; color:var(--text-secondary); letter-spacing:.35px; }
.finance-edit-btn,.finance-icon-btn { width:25px; height:25px; border:0; border-radius:12px; background:transparent; color:var(--text-secondary); cursor:pointer; }
.finance-edit-btn:hover,.finance-icon-btn:hover { background:var(--bg-card-hover); color:var(--text-primary); }
.finance-metric-card .finance-edit-btn { position:absolute; top:10px; right:10px; }
.finance-metric-value { order:2; font-size:22px; font-weight:800; }
.finance-metric-meta { order:4; display:flex; justify-content:space-between; gap:6px; margin-top:9px; font-size:9px; color:var(--text-secondary); width:100%; }
.finance-metric-note { order:6; font-size:9px; color:var(--text-secondary); margin-top:7px; line-height:1.4; }
.finance-metric-card .finance-progress { order:5; width:100%; }
.finance-positive { color:var(--invest-gain,#22c55e)!important; font-style:normal; }
.finance-negative { color:var(--invest-loss,#ef4444)!important; font-style:normal; }
...
.finance-panel h3,.finance-forecast-panel h3 { margin:3px 0 4px; font-size:15px; }
...
.finance-recurring-total { font-size:30px; font-weight:900; margin-top:14px; }
.finance-recurring-total span { font-size:11px; font-weight:600; color:var(--text-secondary); }
...
.finance-forecast-grid strong { font-size:20px; font-weight:800; color:#ffffff; }
...
.planner-timeline { display:flex; flex-direction:column; }
.planner-item { display:grid; grid-template-columns:52px 14px 1fr 28px; align-items:flex-start; gap:12px; padding:12px 0; border-bottom:1px solid var(--border); position:relative; }
.planner-item:last-child { border-bottom:none; }
.planner-item-time { font-family:'JetBrains Mono',monospace; font-size:12px; font-weight:600; color:var(--text-primary); padding-top:2px; }
.planner-item-dot { width:9px; height:9px; border-radius:50%; background:var(--accent); margin:5px auto 0; position:relative; }
.planner-item-dot::after { content:''; position:absolute; top:12px; left:50%; transform:translateX(-50%); width:1px; height:34px; background:var(--border-strong); }
.planner-item:last-child .planner-item-dot::after { display:none; }
.planner-item-title { font-size:13px; font-weight:600; color:var(--text-primary); }
.planner-item-notes { font-size:11px; color:var(--text-secondary); margin-top:3px; line-height:1.4; }
.planner-item-past { opacity:0.45; }
.planner-item-delete { background:none; border:none; color:var(--text-secondary); cursor:pointer; font-size:15px; padding:0; }
.planner-item-delete:hover { color:var(--text-primary); }
...
.collectibles-table-wrap .media-card.collectible-card { height:auto; padding:8px 8px; gap:3px; }
.collectibles-table-wrap .media-card-icon { width:22px; height:22px; font-size:10px; }
.collectibles-table-wrap .media-card-title { font-size:11px; min-height:0; line-height:1.25; }
...
.collectibles-table th { text-align:left; font-size:10px; text-transform:uppercase; letter-spacing:.4px; color:var(--text-secondary); font-weight:700; padding:12px 16px; border-bottom:1px solid var(--border-strong); white-space:nowrap; }
...
.collectibles-cat-badge { background:var(--badge-default-bg); color:var(--badge-default-text); font-size:10px; font-weight:700; padding:3px 9px; border-radius:24px; }
...
/* ============================================================
   GRÁFICA DE FINANZAS (Efectivo / Ingresado / Emergencia)
   ============================================================ */
.finance-chart-panel .finance-panel-head { margin-bottom:6px; }
.finance-chart-legend { display:flex; gap:16px; flex-wrap:wrap; margin:6px 0 14px; font-size:11px; color:var(--text-secondary); }
.finance-chart-legend span { display:inline-flex; align-items:center; gap:6px; }
.finance-chart-legend i { width:14px; height:2px; border-radius:2px; display:inline-block; background:var(--text-primary); }
...
/* Toolbar (sustituye a la cabecera de texto de Finanzas) */
/* Fila superior: gráfica flotante + tarjeta de patrimonio operativo */
.finance-hero-row { display:grid; grid-template-columns:2.2fr 1fr; gap:20px; align-items:stretch; margin-bottom:18px; }
.finance-floating-chart-wrap { display:flex; flex-direction:column; justify-content:center; padding:6px 0; background:none; border:none; box-shadow:none; }
.finance-floating-svg-wrap { overflow:visible; }
.finance-floating-single { display:flex; gap:22px; flex-wrap:wrap; }
.finance-floating-single div { display:flex; flex-direction:column; gap:3px; }
.finance-floating-single span { font-size:10px; color:var(--text-secondary); }
.finance-floating-single strong { font-size:20px; font-weight:800; }
.finance-networth-card { position:relative; }
.finance-networth-edit-btn { position:absolute; top:14px; right:14px; color:rgba(255,255,255,0.5); }
.finance-networth-edit-btn:hover { color:#fff; background:rgba(255,255,255,0.08); }

/* Cuentas: tarjetas minimalistas y borrosas (reutiliza .collectible-card) */
.finance-accounts-section { margin-top:4px; margin-bottom:14px; }
.finance-accounts-row { display:flex; gap:12px; flex-wrap:wrap; }
.finance-account-card { flex:1 1 200px; min-width:180px; max-width:260px; background:var(--bg-card); border:1px solid var(--border); border-radius:18px; padding:16px 18px; cursor:pointer; transition:border-color .15s ease; }
.finance-account-card:hover { border-color:var(--border-strong); }
.finance-account-note { font-size:9px; color:var(--text-secondary); margin-top:6px; line-height:1.4; }
.finance-account-card-readonly { cursor:pointer; }
.finance-account-card-readonly:hover { border-color:var(--border-strong); }

/* Tarjeta "camino al objetivo" (verde) */
.finance-goal-bar { display:flex; align-items:center; gap:12px; border:1px solid var(--border); border-radius:16px; padding:10px 16px; margin-bottom:14px; }
.finance-goal-bar-icon { font-size:15px; flex-shrink:0; }
.finance-goal-bar-text { flex:1; font-size:11px; color:var(--text-secondary); min-width:0; }
.finance-goal-bar-text strong { color:var(--text-primary); font-weight:700; }
.finance-goal-bar-progress { flex:0 0 140px; margin:0; }
@media(max-width:600px){ .finance-goal-bar{flex-wrap:wrap} .finance-goal-bar-progress{flex-basis:100%} }

.finance-dashboard-foot-actions { display:flex; justify-content:center; margin-top:6px; }
...
.studies-subjects-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:12px; margin-top:12px; }
.studies-subject-card { background:var(--bg-card); border:1px solid var(--border); border-radius:16px; padding:16px; transition:border-color .15s ease; }
.studies-subject-card:hover { border-color:var(--border-strong); }
.studies-subject-link { display:inline-block; font-weight:700; font-size:15px; color:#3b82f6; text-decoration:underline; text-underline-offset:2px; cursor:pointer; }
.studies-subject-link:hover { color:#60a5fa; }
.studies-subject-avg { font-size:12px; font-weight:700; color:var(--text-primary); margin-top:4px; }
.studies-subject-block { margin-top:12px; border:1px solid var(--border); border-radius:12px; padding:10px 12px; }
.studies-subject-block-head { font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:.5px; color:var(--text-secondary); margin-bottom:6px; }
.studies-subject-item-line { font-size:12px; color:var(--text-primary); line-height:1.6; }
.studies-subject-block-empty { font-size:11px; color:var(--text-muted); }
.studies-modal-block { margin:14px 0; }
.studies-item-row { display:flex; gap:6px; align-items:center; margin-bottom:6px; }
.studies-item-row button { background:none; border:0; color:#dc2626; cursor:pointer; font-size:14px; }
...
.studies-day-head { font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:.4px; color:var(--text-secondary); text-align:center; padding:8px 4px; }
...
.studies-hour-cell { position:absolute; left:2px; right:2px; display:flex; flex-direction:column; gap:2px; padding:2px 0; box-sizing:border-box; overflow:hidden; }
.studies-block { flex:1; min-height:0; display:flex; justify-content:space-between; align-items:center; gap:4px; background:var(--bg-card-hover); border-radius:8px; padding:4px 6px; font-size:9.5px; font-weight:700; line-height:1.25; overflow:hidden; transition:background .3s,color .3s; box-sizing:border-box; }
...
.studies-block-now { background:#000; color:#fff; font-weight:700; }
.studies-block-now button { color:#fff; }
.studies-now-line { position:absolute; left:0; right:0; height:1px; background:rgba(239,68,68,0.55); pointer-events:none; z-index:2; }
.studies-now-dot { position:absolute; left:-4px; top:50%; transform:translate(-100%,-50%); width:6px; height:6px; border-radius:50%; background:#ef4444; }
.studies-day-add { position:absolute; right:4px; bottom:4px; width:20px; height:20px; background:none; border:1px dashed var(--border-strong); border-radius:6px; font-size:11px; line-height:1; color:var(--text-secondary); cursor:pointer; display:flex; align-items:center; justify-content:center; z-index:1; }
...
.link-card-title { font-weight:700; font-size:14px; color:var(--text-primary); padding-right:32px; }
.link-card-url { display:block; margin-top:8px; font-size:12px; font-weight:700; color:#3b82f6; text-decoration:underline; text-underline-offset:2px; word-break:break-all; }
...
.link-card-delete { color:#7f1d1d; font-size:15px; font-weight:900; }
.link-card-delete:hover { background:#fee2e2; }
.link-card-edit { color:#1a1a1a; }
.link-card-edit:hover { background:#e5e5e5; }
.links-category-head { display:flex; align-items:center; gap:8px; font-size:13px; font-weight:800; color:var(--text-primary); margin-bottom:4px; }
...
.bitacora-username-link { color:#3b82f6; text-decoration:underline; text-underline-offset:2px; cursor:pointer; font-weight:800; }
...
.goals-summary strong { font-size:26px; font-weight:900; color:var(--text-primary); line-height:1; }
...
.project-overdue { color:#f87171; font-weight:700; }
...
.project-roadmap-check { flex-shrink:0; width:21px; height:21px; border-radius:50%; border:2px solid var(--border-strong); display:flex; align-items:center; justify-content:center; font-size:10px; font-weight:700; color:var(--text-muted); background:var(--bg-card); transition:all .15s ease; z-index:1; }
...
.project-feature-chip b { color:var(--text-primary); font-weight:700; }
...
.project-card { background:var(--bg-card); border:1px solid var(--border); border-top:3px solid var(--text-secondary); border-radius:16px; padding:16px; cursor:pointer; transition:border-color .15s ease,transform .15s ease; }
...
.project-card-title { font-weight:700; font-size:15px; color:var(--text-primary); line-height:1.3; }
.project-card-meta { display:flex; flex-wrap:wrap; align-items:center; gap:8px; margin-top:8px; font-size:11px; color:var(--text-secondary); }
.project-card-desc { font-size:12px; color:var(--text-secondary); margin-top:8px; line-height:1.4; overflow:hidden; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; }
...
.kanban-column-head { font-size:11px; font-weight:800; text-transform:uppercase; letter-spacing:.5px; color:var(--text-secondary); margin-bottom:10px; display:flex; justify-content:space-between; }
.kanban-card {
...
.kanban-card-title { font-weight:700; font-size:13px; color:var(--text-primary); }
...
.event-hero-kicker { font-size:11px; font-weight:800; text-transform:uppercase; letter-spacing:.6px; margin-bottom:8px; }
.event-hero-title { font-size:22px; font-weight:800; color:var(--text-primary); letter-spacing:-.3px; }
...
.events-month-label { font-size:11px; font-weight:800; text-transform:uppercase; letter-spacing:.5px; color:var(--text-secondary); margin:20px 0 10px; }
.events-month-label:first-child { margin-top:0; }
.events-month-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(190px,1fr)); gap:10px; }
.event-card { background:rgba(255,255,255,0.09); color:#fff; border:3px solid rgba(255,255,255,0.07); border-radius:14px; padding:12px 14px; cursor:pointer; transition:transform .15s ease, box-shadow .15s ease; }
.event-card:hover { transform:translateY(-2px); box-shadow:0 6px 16px rgba(0,0,0,.35); }
.event-card-top { display:flex; justify-content:flex-end; align-items:center; gap:8px; }
.event-card-date { font-size:10px; font-weight:700; color:rgba(255,255,255,0.55); white-space:nowrap; }
.event-card-title { font-size:13px; font-weight:700; margin-top:6px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.event-card-place { font-size:11px; color:rgba(255,255,255,0.6); margin-top:3px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

.media-month-label { display:block; font-size:13px; font-weight:800; color:#3b82f6; text-decoration:underline; text-underline-offset:3px; text-transform:lowercase; margin:18px 0 10px; }
...
.day-entry-title { font-weight:700; font-size:14px; color:var(--text-primary); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.finance-correction-month { font-size:11px; font-weight:800; text-transform:capitalize; margin-bottom:6px; }
...
.finance-account-label { font-size:10px; text-transform:uppercase; letter-spacing:.4px; font-weight:800; color:var(--text-secondary); }
.finance-account-value { font-size:20px; font-weight:800; margin-top:4px; }
.finance-account-value span { font-size:10px; font-weight:600; color:var(--text-secondary); }
...
.tags-view-chip { display:inline-flex; align-items:center; gap:4px; border:1px solid var(--border); background:var(--bg-card); color:var(--text-primary); border-radius:12px; font-size:12px; font-weight:700; padding:8px 12px; cursor:pointer; box-shadow:var(--shadow); transition:transform .15s ease, background .15s ease, border-color .15s ease; }
...
.apuntes-window-title { font-size:13px; font-weight:800; letter-spacing:.2px; color:var(--text-primary, #fff); }
